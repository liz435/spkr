...
// 💥 Improved Shockwave Effect (Based on Shadertoy)
class ImprovedShockwaveEffect extends Effect {
  private _intensity: number;
  private _waveWidth: number;
  ...
  constructor({
    ...
    const fragmentShader = `
      ...
      void mainImage(const in vec4 inputColor, const in vec2 uv, out vec4 outputColor) {
        ...
        vec3 waveParams = vec3(10.0, 0.8, waveWidth);
    ...
    super('ImprovedShockwaveEffect', fragmentShader, {
      blendFunction: BlendFunction.NORMAL,
      uniforms: new Map([
        ['time', new THREE.Uniform(0.0)],
        ['intensity', new THREE.Uniform(intensity)],
        ['waveWidth', new THREE.Uniform(waveWidth)],
        ['waveStrength', new THREE.Uniform(waveStrength)],
        ['resolution', new THREE.Uniform(new THREE.Vector2(1920, 1080))]
      ] as any)
    });
  ...
  updateWaveWidth(waveWidth: number) {
    this.uniforms.get('waveWidth')!.value = waveWidth;
    this._waveWidth = waveWidth;
  }

  updateWaveStrength(waveStrength: number) {
    this.uniforms.get('waveStrength')!.value = waveStrength;
    this._waveStrength = waveStrength;
  }
  ...
  get intensity() { return this._intensity; }
  get waveWidth() { return this._waveWidth; }
  get waveStrength() { return this._waveStrength; }
}
...
// 💥 Improved Shockwave Effect Component
function ImprovedShockwaveComponent({
  intensity,
  waveWidth,
  waveStrength,
  enabled
  ...
  const { size: canvasSize } = useThree();
  const effectRef = useRef<ImprovedShockwaveEffect | null>(null);

  const effect = useMemo(() => {
    const shockwaveEffect = new ImprovedShockwaveEffect({ intensity, waveWidth, waveStrength });
    effectRef.current = shockwaveEffect;
    return shockwaveEffect;
  }, [intensity, waveWidth, waveStrength]);

  useEffect(() => {
    if (effectRef.current) {
      effectRef.current.updateIntensity(intensity);
      effectRef.current.updateWaveWidth(waveWidth);
      effectRef.current.updateWaveStrength(waveStrength);
      effectRef.current.updateResolution(canvasSize.width, canvasSize.height);
    }
  }, [intensity, waveWidth, waveStrength, canvasSize.width, canvasSize.height]);
...
interface ShaderFiltersProps {
  ...
  improvedShockwaveEnabled?: boolean;
  improvedShockwaveIntensity?: number;
  improvedShockwaveWidth?: number;
  improvedShockwaveStrength?: number;
...
export function ShaderFilters({
  ...
  shockwaveSize = 0.1,
  shockwaveSpeed = 1.0,
  improvedShockwaveEnabled = false,
  improvedShockwaveIntensity = 1.0,
  improvedShockwaveWidth = 0.1,
  improvedShockwaveStrength = 10.0,
  ...
  if (improvedShockwaveEnabled) {
    effects.push(
      <ImprovedShockwaveComponent
        key="improvedShockwave"
        intensity={improvedShockwaveIntensity}
        waveWidth={improvedShockwaveWidth}
        waveStrength={improvedShockwaveStrength}
        enabled={improvedShockwaveEnabled}
      />
    );
  }
